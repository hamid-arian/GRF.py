...
class CausalSurvivalSplittingRule:
    def __init__(self, max_num_unique_values, min_node_size, alpha, imbalance_penalty):
        ...
        self.failure_count = np.zeros(max_num_unique_values, dtype=int)
    ...
    def find_best_split(self, data, node, possible_split_vars, responses_by_sample, samples, split_vars, split_values, send_missing_left):
        ...
        min_child_size_survival = max(int(np.ceil(num_samples * self.alpha)), 1)
        ...
        for var in possible_split_vars:
            self.find_best_split_value(data, node, var, num_samples, weight_sum_node, sum_node, mean_z_node,
                                       num_node_small_z, sum_node_z, sum_node_z_squared, num_failures_node,
                                       min_child_size, min_child_size_survival, best_value, best_var,
                                       best_decrease, best_send_missing_left, responses_by_sample, samples)
    ...
    def find_best_split_value(self, data, node, var, num_samples, weight_sum_node, sum_node, mean_node_z,
        ...
        for i in range(num_samples - 1):
            ...
            if np.isnan(sample_value):
                ...
                if data.is_failure(sample):
                    self.failure_count[split_index] += 1
        ...
        num_failures_left = num_failures_missing
        ...
        for send_left in [True, False]:
            ...
            for i in range(num_splits):
                ...
                num_failures_left += self.failure_count[i]
                ...
                if num_right_small_z < self.min_node_size or num_right_large_z < self.min_node_size:
                    break
